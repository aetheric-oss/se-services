...
Each microservice will populate the following files:

File | Description
---- | -----
`openapi/types.rs` | Structures and types used for this microservice's REST server API
`server/src/main.rs` | Implementation of REST server interfaces to this microservice
`client-rest/src/lib.rs` | The `openapi/types.rs` file bundled into a library

The `client-rest` crate for each microservice (e.g. `svc-cargo-client-rest`) can be published to a registry.
...
Each microservice will populate the following files for gRPC:

File | Description
---- | -----
`proto/grpc.proto` | The gRPC interfaces for this microservice
`server/src/grpc.rs` | Autogenerated Server Stubs
`server/src/main.rs` | Implements Server Stubs
`client-grpc/src/grpc.rs` | Autogenerated Client Code
...
│   ├── Cargo.toml
│   └── src
│       ├── grpc.rs # autogenerated client stubs during `cargo build`
│       └── lib.rs
├── proto/
...
└── server
    ├── Cargo.toml
    ├── src
    │   ├── grpc.rs # autogenerated server stubs during `cargo build`
    │   └── main.rs
```
Details:
- A service codebase shall contain two Rust crates, defined in the directories `server` and `client-grpc`.
  - The `server` crate is an executable binary
  - The `client-grpc` crate is a library
...
- During `cargo build`, the protobuf files will be used to autogenerate gRPC server and client stubs.
  ...
  - The autogenerated client code is created in the `client-grpc/src` directory.
- Each crate is then compiled
- CI/CD will separately publish these crates to a registry.
  - `svc-scheduler` will publish `svc-scheduler` (server binary) and `svc-scheduler-client-grpc` (client library).
...
[dependencies]
svc-scheduler-client-grpc = 0.1.0
```
